...
class AudioPlayerFullSuraWidgetState extends State<AudioPlayerFullSuraWidget> {
  ...
  void _playPause() {
    if (readFullSuraController.playerState == PlayerState.playing) {
      _audioPlayer.pause();
    } else if (readFullSuraController.playerState == PlayerState.paused) {
      _audioPlayer.resume();
    } else {
        ...
        return Container(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              Builder(
                builder: (context) {
                  double value = readFullSuraController.playerPosition?.inSeconds.toDouble() ?? 0;
              ...
              Row(
                ...
                children: <Widget>[
                  Text(
                    _formatDuration(readFullSuraController.playerPosition ?? const Duration()),
                ...
                children: <Widget>[
                  IconButton(
                    icon: Builder(
                      builder: (context) {
                        return Icon(
                          readFullSuraController.playerState == PlayerState.playing
